...
DEFAULT_BASE_DIR = "/Users/fudhailsayed/prololab/data/datasets/pHGG_scRNA_anndata/scRNA"
DEFAULT_OUT_DIR  = "/Users/fudhailsayed/prololab/figures/neftal"
...
COUNTS_LAYER_CANDIDATES = ["counts", "raw_counts", "rna_counts", "count", "Count", "Counts"]
...
GENESETS = {
    "MES1": [
        "CHI3L1","ANXA2","ANXA1","CD44","VIM","MT2A","C1S","NAMPT",
        "EFEMP1","C1R","SOD2","IFITM3","TIMP1","SPP1","A2M","S100A11",
        "MT1X","S100A10","FN1","LGALS1","S100A16","CLIC1","MGST1",
        "RCAN1","TAGLN2","NPC2","HEY1","SERPING1","C8orf4","EMP1",
    ...
    "MES2": [
        "HILPDA","ADM","DDIT3","NDRG1","HERPUD1","DNAJB9","TRIB3","ENO2",
        "AKAP12","SQSTM1","MT1X","ATF3","NAMPT","NRN1","SLC2A1","BNIP3",
        "LGALS3","INSIG2","IGFBP3","PPP1R15A","VIM","PLOD2","GBE1",
        "SLC2A3","FTL","WARS","ERO1L","XBP1","HSPA5","GDF15","ANXA2",
        "EPAS1","LDHA","P4HA1","SERTAD1","PFKP","PGK1","EGLN3",
        "SLC6A6","CA9","BNIP3L","RPL21","TRAM1","UFM1","ASNS",
        "GOLT1B","ANGPTL4","SLC39A14","CDKN1A","HSPA9"
    ],
    "AC": [
        "CST3","S100B","SLC1A3","HEPN1","HOPX","MT3","SPARCL1","MLC1",
        "GFAP","FABP7","BCAN","PON2","METTL7B","SPARC","GATM","RAMP1",
        "PMP2","AQP4","DBI","EDNRB","CLU","PMP22","ATP1A2","S100A16",
        "CA10","PCDHGC3","TTYH1","NDRG2","PRCP","ATP1B2","AGT","PLTP",
        "GPM6B","F3","RAB31","PPAP2B","ANXA5","TSPAN7"
    ],
    "OPC": [
        "BCAN","PLP1","GPR17","FIBIN","LHFPL3","OLIG1","PSAT1","SCRG1",
        "OMG","APOD","SIRT2","TNR","THY1","PHYHIPL","SOX2-OT","NKAIN4",
        "LPPR1","PTPRZ1","VCAN","DBI","PMP2","CNP","TNS3","LIMA1",
        "PCDHGC3","CNTN1","SCD5","P2RX7","CADM2","TTYH1","FGF12",
        "TMEM206","NEU4","FXYD6","RNF13","RTKN","GPM6B","LMF1",
        "ALCAM","PGRMC1","HRASLS","BCAS1","RAB31","PLLP","FABP5",
        "NLGN3","SERINC5","EPB41L2","GPR37L1"
    ],
    "NPC1": [
        "DLL3","DLL1","SOX4","TUBB3","HES6","TAGLN3","NEU4","MARCKSL1",
        "CD24","STMN1","TCF12","BEX1","OLIG1","MAP2","FXYD6","PTPRS",
        "MLLT11","NPPA","BCAN","MEST","ASCL1","BTG2","DCX","NXPH1",
        "HN1","PFN2","SCG3","MYT1","CHD7","GPR56","TUBA1A","PCBP4",
        "ETV1","SHD","TNR","AMOTL2","DBN1","HIP1","ABAT","ELAVL4",
        "LMF1","GRIK2","SERINC5","TSPAN13","ELMO1","GLCCI1","SEZ6L",
        "LRRN1","SEZ6","SOX11"
    ],
    "NPC2": [
        "STMN2","DCX","SOX11","ELAVL4","DLX6-AS1","CD24","RND3","HMP19",
        "TUBB3","MIAT","DCX","NSG1","ELAVL4","MLLT11","DLX6-AS1","SOX11",
        "NREP","FNBP1L","TAGLN3","STMN4","DLX5","SOX4","MAP1B","RBFOX2",
        "IGFBPL1","STMN1","HN1","TMEM161B-AS1","DPYSL3","SEPT3","PKIA",
        "ATP1B1","DYNC1I1","CD200","SNAP25","PAK3","NDRG4","KIF5A",
        "UCHL1","ENO2","DDAH2","TUBB2A","LBH","LOC150568","TCF4",
        "GNG3","NFIB","DPYSL5","CRABP1","DBN1","NFIX","CEP170","BLCAP"
    ],
    "G1S": [
        "RRM2","PCNA","KIAA0101","HIST1H4C","MLF1IP","GMNN","RNASEH2A",
        "MELK","CENPK","TK1","TMEM106C","CDCA5","CKS1B","CDC45","MCM3",
        "CENPM","AURKB","PKMYT1","MCM4","ASF1B","GINS2","MCM2","FEN1",
        "RRM1","DUT","RAD51AP1","MCM7","CCNE2","ZWINT"
    ],
    "G2M": [
        "CCNB1","CDC20","CCNB2","PLK1","CCNA2","CKAP2","KNSTRN","RACGAP1",
        "CDCA3","TROAP","KIF2C","AURKA","CENPF","KPNA2","KIF20A","ECT2",
        "BUB1","CDCA8","BUB1B","TACC3","TTK","TUBA1C","NCAPD2","ARL6IP1",
        "KIF4A","CKAP2L","MZT1","KIFC1","SPAG5","ANP32E","KIF11","PSRC1",
        "TUBB4B","SMC4","MXD3","CDC25B","OPIP5","REEP4","FOXM1","TMPO",
    ...
    if prefer_raw and adata.raw is not None and adata.raw.X is not None:
        ...
        if mn >= 0:
            if mx > 50:
                ...
                print("[INFO] prefer_raw: using adata.raw.X (counts-like) → created neftal_log1p")
            else:
                adata.layers["neftal_log1p"] = Xr
                print("[INFO] prefer_raw: using adata.raw.X as neftal_log1p (already log-like)")
            return "neftal_log1p"
...
def malignant_mask_from_cnv(
    adata,
    cnv_col: str,
    cnv_thr: Optional[float],
    cnv_quantile: float
) -> Tuple[Optional[np.ndarray], Optional[float]]:
    if cnv_col not in adata.obs.columns:
        return None, None
    cnv = pd.to_numeric(adata.obs[cnv_col], errors="coerce").values
    thr = float(cnv_thr) if cnv_thr is not None else float(np.nanpercentile(cnv, cnv_quantile))
    mask = cnv >= thr
    return mask, thr

def find_samples(base_dir):
    folders = sorted([p for p in glob.glob(os.path.join(base_dir, "SCPCS*")) if os.path.isdir(p)])
    if not folders:
...
def load_one_sample(
    ...
    if col is not None:
        ...
        print(f"[WARN] {sample_id}: No gene symbol column found in ad.var; using existing var_names")
    ...
    for key in ["MES1","MES2","AC","OPC","NPC1","NPC2","G1S","G2M"]:
        ...
        if n_present == 0:
            print(f"[WARN] {sample_id}: score_{key}: 0/{n_total} genes found (score set to zeros)")
        report_rows.append({"sample_id": sample_id, "program": key, "n_present": n_present, "n_total": n_total})

    ad.obs["score_MES"] = 0.5*(ad.obs["score_MES1"].values + ad.obs["score_MES2"].values)
    ad.obs["score_NPC"] = 0.5*(ad.obs["score_NPC1"].values + ad.obs["score_NPC2"].values)
    ...
    mask, thr = malignant_mask_from_cnv(ad, cnv_col=cnv_col, cnv_thr=cnv_thr, cnv_quantile=cnv_quantile)
    if mask is not None:
        kept = int(np.nansum(mask))
        tot = ad.n_obs
        print(f"[INFO] {sample_id}: malignant(CNV) using {cnv_col}, thr={thr:.2f}, kept {kept}/{tot}")
        ad.obs["is_malignant_cnv"] = mask
        ad.uns["cnv_thr"] = thr
    else:
        ad.obs["is_malignant_cnv"] = False
        print(f"[WARN] {sample_id}: CNV column '{cnv_col}' not found → malignant filtering unavailable")
...
def get_gene_vector(adata, gene, layer_key):
    ...
    if gene not in adata.var_names:
        ...
        return None
    X = adata.layers[layer_key] if (layer_key is not None and layer_key in adata.layers) else adata.X
...
def plot_left_stacked_bar(ax, summary: pd.DataFrame, title: str):
    ...
    for quad, frac in zip(order, fracs):
        ...
        if frac >= 0.06:
            ax.text(0, bottom + frac / 2.0, f"{quad}\n{frac*100:.1f}%", ha="center", va="center", fontsize=9)
        bottom += frac
...
def high_expr_summary_df(
    ...
    if v is None:
        return pd.DataFrame([{
            "group": label, "gene": gene, "mode": mode, "threshold_log1p": np.nan,
            "n_cells": int(adata.n_obs), "n_high": 0, "pct_high": 0.0
        }])
    ...
    return pd.DataFrame([{
        "group": label, "gene": gene, "mode": mode, "threshold_log1p": thr_used,
        "n_cells": n, "n_high": n_high, "pct_high": pct_high
    }])
...
def exprbin_quadrant_table(
    ...
    if "neftal_quadrant" not in adata.obs.columns:
        adata.obs["neftal_quadrant"] = assign_quadrants(
            adata.obs["neftal_x"].values,
            adata.obs["neftal_y"].values
        )
    ...
    quad_order = ["MES-like", "OPC-like", "NPC-like", "AC-like"]  # bottom-to-top-ish; stable order
    # We'll use the same quad names as elsewhere:
    quad_order = ["OPC-like", "NPC-like", "AC-like", "MES-like"]

    df = pd.DataFrame({
        "bin_id": bin_ids,
        "quadrant": adata.obs["neftal_quadrant"].values
    })
    ...
    ct["pct_within_bin"] = np.where(ct["bin_total"] > 0, ct["n_cells"] / ct["bin_total"] * 100.0, 0.0)

    ct["group"] = group_label
    ct["gene"] = gene
    ct["bin_label"] = ct["bin_id"].map({i: bin_labels[i] for i in range(len(bin_labels))})
    ct["bin_left"] = ct["bin_id"].map({i: edges[i] for i in range(len(bin_labels))})
    ct["bin_right"] = ct["bin_id"].map({i: edges[i+1] for i in range(len(bin_labels))})
...
def plot_exprbin_quadrant_stackedbars(
    ...
    # wide matrix: rows=bins, cols=quadrants
    mat = []
    ...
    # legend
    ax.legend(quad_order, title="Quadrant", bbox_to_anchor=(1.02, 1), loc="upper left", borderaxespad=0.0)
...
def plot_neftal_with_quadrants(
    ...
    if color_gene is not None:
        ...
        if expr is not None:
            ...
            cb.set_label(f"log1p({color_gene}) (clipped {color_clip_pct[0]}–{color_clip_pct[1]}%)")
        else:
    ...
    ax.text(0.02, 0.95, "OPC-like", transform=ax.transAxes, ha="left", va="top", fontsize=13)
    ax.text(0.98, 0.95, "NPC-like", transform=ax.transAxes, ha="right", va="top", fontsize=13)
    ax.text(0.02, 0.05, "AC-like",  transform=ax.transAxes, ha="left", va="bottom", fontsize=13)
    ax.text(0.98, 0.05, "MES-like", transform=ax.transAxes, ha="right", va="bottom", fontsize=13)

    if high_text:
        ax.text(0.02, 0.02, high_text, transform=ax.transAxes, ha="left", va="bottom", fontsize=10)

    ax.set_title(title, fontsize=14)
    ax.set_xlabel("Relative meta-module score  [signed log2(|NPC − OPC| + 1)]", fontsize=12)
    ax.set_ylabel("Relative meta-module score  [signed log2(|(OPC+NPC) − (AC+MES)| + 1)]", fontsize=12)
...
def build_argparser():
    ...
    p.add_argument("--base_dir", default=DEFAULT_BASE_DIR, help="Base directory containing SCPCS* folders")
    p.add_argument("--out_dir",  default=DEFAULT_OUT_DIR,  help="Output directory for plots and report")

    p.add_argument("--color_gene", default=DEFAULT_COLOR_GENE, help="Gene to color points by (set to 'None' to disable)")
    p.add_argument("--cmap", default=DEFAULT_CMAP, help="Matplotlib colormap name")

    p.add_argument("--prefer_raw", action="store_true", help="Prefer adata.raw.X for scoring if present")
    p.add_argument("--do_malignant_filter", action="store_true", help="Also write malignant-only plot (if CNV col exists)")

    p.add_argument("--cnv_col", default=DEFAULT_CNV_COL, help="CNV column in adata.obs used for malignant filter")
    p.add_argument("--cnv_quantile", type=float, default=90.0, help="Percentile for CNV threshold if cnv_thr not set")
    p.add_argument("--cnv_thr", type=float, default=None, help="Manual CNV threshold (overrides cnv_quantile)")

    p.add_argument("--seed", type=int, default=0, help="Random seed (used only for stable ordering)")
    p.add_argument("--file_glob", default=DEFAULT_FILE_GLOB, help="Pattern for .h5ad file inside each SCPCS folder")
    p.add_argument("--no_standardize", action="store_true", help="Disable centering/scaling of axes")

    # Old: High expressing
    p.add_argument("--high_gene", default=None, help="Gene for % high expressing calculation (default: color_gene)")
    p.add_argument("--high_mode", default="pct", choices=["pct", "abs", "nonzero"],
                   help="How to define high expressers: pct=percentile, abs=absolute log1p threshold, nonzero")
    p.add_argument("--high_pct", type=float, default=90.0, help="Percentile for high (used if high_mode=pct)")
    p.add_argument("--high_thr", type=float, default=1.0, help="Absolute threshold on log1p(expr) (used if high_mode=abs)")

    # NEW: Expression bins stacked-bars
    p.add_argument("--bin_gene", default=None, help="Gene for binning (default: color_gene)")
    p.add_argument("--bin_mode", default="equalwidth_clip",
                   choices=["equalwidth_clip", "equalwidth", "quantile", "edges"],
                   help="How to create bins for log1p(expr)")
    p.add_argument("--bin_n", type=int, default=4, help="Number of bins (ignored if bin_mode=edges)")
    p.add_argument("--bin_edges", default=None, help="Comma-separated edges for bin_mode=edges, e.g. '0,0.4,0.8,1.2,1.6'")
    p.add_argument("--bin_y", default="counts", choices=["counts", "pct"],
                   help="Y-axis for bin stacked bars: counts (#cells) or pct (% within bin)")
    p.add_argument("--bin_clip_pct_low", type=float, default=DEFAULT_COLOR_CLIP_PCT[0],
                   help="Lower percentile for equalwidth_clip range")
    p.add_argument("--bin_clip_pct_high", type=float, default=DEFAULT_COLOR_CLIP_PCT[1],
...
def main():
    ...
    ad_all = sc.concat(all_ads, join="outer", label="sample_id", keys=[ad.obs["sample_id"].iloc[0] for ad in all_ads])
    ...
    if layer_for_color is None:
        print("[WARN] pooled object has no neftal_log1p layer; gene extraction uses ad_all.X")
    ...
    if high_gene is not None:
        ...
        thr_used = float(hi_all["threshold_log1p"].iloc[0]) if not np.isnan(hi_all["threshold_log1p"].iloc[0]) else np.nan
        ...
        if args.high_mode == "pct":
            ...
            high_text_all = f"{high_gene}: log1p >= {args.high_thr:.3f} → {pct_high:.2f}% high"
        else:
            high_text_all = f"{high_gene}: expr > 0 → {pct_high:.2f}% high"

    # neftal plot (ALL)
    out_all = os.path.join(out_dir, f"neftal_ALLSAMPLES_ALL_{color_gene if color_gene else 'noclr'}.png")
    ...
    if bin_gene is not None:
        ...
        if not tab_all.empty:
            tab_all_csv = os.path.join(out_dir, f"exprbin_quadrant_counts_ALL_{bin_gene}.csv")
            tab_all.to_csv(tab_all_csv, index=False)
            print(f"[DONE] Saved expr-bin quadrant table: {tab_all_csv}")

            out_bar_all = os.path.join(out_dir, f"exprbin_quadrant_stackedbar_ALL_{bin_gene}.png")
    ...
    if args.do_malignant_filter and "is_malignant_cnv" in ad_all.obs.columns:
        ...
        if mal.n_obs == 0:
            print("[WARN] No malignant cells detected after filtering. Skipping malignant plot.")
        else:
            ...
            if high_gene is not None:
                ...
                hi_mal_csv = os.path.join(out_dir, f"high_expr_summary_MALIGNANT_{high_gene}.csv")
                hi_mal.to_csv(hi_mal_csv, index=False)
                print(f"[DONE] Saved high-expressing summary: {hi_mal_csv}")

                thr_used = float(hi_mal["threshold_log1p"].iloc[0]) if not np.isnan(hi_mal["threshold_log1p"].iloc[0]) else np.nan
            ...
            out_mal = os.path.join(out_dir, f"neftal_ALLSAMPLES_MALIGNANT_{color_gene if color_gene else 'noclr'}.png")
            plot_neftal_with_quadrants(
                mal,
                out_mal,
            ...
            if bin_gene is not None:
                ...
                if not tab_mal.empty:
                    tab_mal_csv = os.path.join(out_dir, f"exprbin_quadrant_counts_MALIGNANT_{bin_gene}.csv")
                    tab_mal.to_csv(tab_mal_csv, index=False)
                    print(f"[DONE] Saved expr-bin quadrant table: {tab_mal_csv}")

                    out_bar_mal = os.path.join(out_dir, f"exprbin_quadrant_stackedbar_MALIGNANT_{bin_gene}.png")
                    plot_exprbin_quadrant_stackedbars(
                        tab_mal,
                        out_png=out_bar_mal,
                    ...
                    print("[WARN] Could not compute expr-bin table for MALIGNANT (empty).")
